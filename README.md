...
Afterwards, merge the branch back into `main`. `gradle.properties` which contains the version read by XHDWalletAPIAndroid, XHDWalletAPIJVM and sharedModule will have also been updated by `semantic-release`, ensuring the repository release version on GitHub is in-sync with the versions for the respective modules.
